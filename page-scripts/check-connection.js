document.addEventListener('DOMContentLoaded', function () {
    ...
    // Measure load time
    const loadTime = Math.round(performance.now()); // Time since navigation started
        ...
        fetch(window.location.href, { method: "HEAD" })
            .then(response => {
                ...
                if (!response.ok && isConnected) {
                    ...
                    isConnected = false;
                    ping = "--";
                    connectionText.innerHTML = `Connection: <span style='color: red;'>Disconnected</span><br>Ping: ${ping}<br>Load Time: ${loadTime} ms`;
                    ...
                    isConnected = true;
                    connectionText.innerHTML = `Connection: <span style='color: ${statusColor};'>${connectionStatus}</span><br>Ping: ${ping} ms<br>Load Time: ${loadTime} ms`;
    ...
    // Update Load Time on Page Load
    connectionText.innerHTML = `Connection: Wait..<br>Ping: Wait..<br>Load Time: ${loadTime} ms`;
});
